...
export function decomposeRequirement(input: string): RequirementDecomposition {
  ...
  const matchedRules = INTENT_RULES.filter((rule) => includesAny(text, rule.terms));
  const enabled = shouldEnableDecomposition(text, matchedRules.length);
